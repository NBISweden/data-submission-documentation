...
* Most of the metadata information resides with the data producer, e.g. NGI, ask them (or the reponsible researcher) to fill in missing values.
...
* The last line indicate data type
  ...
  * For single read .fastq files, begin row with `FASTQ:` and add the full path to where the sequence file resides, make sure that it is gzipped. 
    * **Note**: If paired .fastq reads are to be submitted, add another row (below INSTRUMENT) with `INSERT_SIZE:` and the value of the insert size as an integer.
...
* Download the [latest Webin-CLI version](https://github.com/enasequence/webin-cli/releases/latest), and validate & submit:
    ```
    java -jar /path/to/webin-cli-[version].jar -ascp -context reads -userName Webin-XXX -password [PASSWORD] -manifest ./experiment-manifest.txt -validate
     
    java -jar /path/to/webin-cli-[version].jar -ascp -context reads -userName Webin-XXX -password [PASSWORD] -manifest ./experiment-manifest.txt -submit
    ```
