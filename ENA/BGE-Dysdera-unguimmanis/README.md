...
#### XML
* I created [qqDysUngu-HiFi.tsv](./data/qqDysUngu-HiFi.tsv)
* Run script:
    ```
    ../../../../ERGA-submission/get_submission_xmls/get_ENA_xml_files.py -f qqDysUngu-HiFi.tsv -p ERGA-BGE -o qqDysUngu-HiFi
    ```
...
* Submit both projects and experiment in one go, i.e:
    ```
    curl -u username:password -F "SUBMISSION=@submission.xml" -F "PROJECT=@qqDysUngu-HiFi.study.xml" -F "EXPERIMENT=@qqDysUngu-HiFi.exp.xml" -F "RUN=@qqDysUngu-HiFi.runs.xml" "https://www.ebi.ac.uk/ena/submit/drop-box/submit/"
    ```
* Receipt:
    ```
...
* Run script:
    ...
    ```
* Update qqDysUngu-HiC.exp.xml to reference accession number of previously registered study:
    ```
    <STUDY_REF accession=""/>
    ```
